...
struct memory_t {
  ...
  uint32_t read_ifetch(uint32_t addr) const {
  ...
  uint32_t read_w(uint32_t addr) const {
    const uint32_t addr_lo = addr & mask_lo;
  ...
  uint16_t read_s(uint32_t addr) const {
    const uint32_t addr_lo = addr & mask_lo;
    // test if this is within one chunk
    if (addr_lo <= 0xfffe) {
      // get the chunk
  ...
  uint8_t read_b(uint32_t addr) const {
    // get the chunk
    if (chunk_t *c = chunks[addr >> 16]) {
  ...
  void read(uint8_t *dst, uint32_t addr, uint32_t size) const {
